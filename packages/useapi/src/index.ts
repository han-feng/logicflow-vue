...
import { computed, inject, nextTick, onActivated, onDeactivated, provide, reactive, ref, shallowReactive, toRaw, watch } from 'vue';
import type { GraphData, ModelerContext, ModelType, PropertiesPanelConfig, PropertiesPanelContext, PropertiesPanelData, PropertiesPanelView, ViewerContext } from './types';
...
export function useViewer(model: ModelType): ViewerContext {
  ...
  const viewer: ViewerContext = {
    ...
    getDataObject: function (): GraphData | undefined {
    ...
    getData: function (adapterKey: string = 'default'): string {
      const data = viewer.getDataObject()
      if (adapterKey === 'json.mini') return JSON.stringify(data)
      if (model.adapters) {
        const adapter = model.adapters[adapterKey]
        if (adapter) return adapter.out(data)
      }
      return JSON.stringify(data, null, 2)
    },

    // export
    exportData: function (filename: string = model.name, adapterKey: string = 'json.mini') {
      const text = viewer.getData(adapterKey)
      download(filename, text ? text.toString() : '')
    },

    exportPng: function (filename: string = model.name, backgroundColor?: string) {
    ...
    scale: reactive({
      value: 1,
      percentage: computed(() => {
        let p: number = viewer.scale.value
    ...
    fitView: function (offset: number) {
      viewer.lf?.fitView(offset);
    },

    // logicflow
    initLogicFlow: function (logicflowOptions: any) {
      // TODO: 默认插件
      // const plugins = [
 ...
 */
export function usePropertiesPanelData(isTop: boolean = false): PropertiesPanelData {
  const ctx = inject<PropertiesPanelContext>('properties_panel_context') // 注入属性面板上下文

  if (ctx && isTop) return ctx.modeler.graphData // 如果是顶层，直接返回图数据
